  ...
  const resize = useCallback(
    (e: MouseEvent) => {
      if (isResizing) {
        ...
        const newSize = reverse ? startSize.current - delta : startSize.current + delta;
        const boundedSize = Math.max(minSize, Math.min(maxSize, newSize));
        
        setSize(boundedSize);
        setResizingValue(`${boundedSize}px`);
      }
    },
    [isResizing, minSize, maxSize, direction, reverse, setResizingValue],
  );
